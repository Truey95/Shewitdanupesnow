...
export default function HWDZNCollection() {
  const { data: productsData, isLoading, error } = useProductsByCategory('hwdzn');

  return (
    <div className="min-h-screen w-full" style={{ backgroundColor: '#0000FF' }}>
      <div className="container mx-auto py-16">
        <h1 className="text-6xl font-serif mb-4" style={{ color: '#FFFFFF' }}>
          He Wit Da Zetas Now
        </h1>
        
        <p className="text-xl mb-12" style={{ color: '#FFFFFF', opacity: 0.8 }}>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <div className="aspect-square bg-white/10 rounded-lg p-6 flex items-center justify-center">
              <p className="text-2xl" style={{ color: '#FFFFFF' }}>Coming Soon</p>
            </div>
          </div>
        )}
